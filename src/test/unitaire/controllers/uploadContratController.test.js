...
describe('uploadContrat Controller', () => {
  ...
  it('devrait créer un nouveau contrat si aucun n\'existe pour cette réservation', async () => {
    // Arrange
    ...
    mockPrisma.contrat.create.mockResolvedValue(mockContrat);
    ...
    expect(mockPrisma.contrat.create).toHaveBeenCalledWith({
      data: { reservationId: 123, signature: true }
    });
    
    expect(cloudinary.uploader.upload).toHaveBeenCalledWith('/tmp/contrat.pdf', {
      folder: 'contrats',
      use_filename: true,
    ...
    expect(mockPrisma.media.create).toHaveBeenCalledWith({
      data: {
        url: 'https://cloudinary.com/contrat.pdf',
        type: 'CONTRAT',
    ...
    expect(fs.unlinkSync).toHaveBeenCalledWith('/tmp/contrat.pdf');
  ...
  it('devrait utiliser un contrat existant sans en créer un nouveau', async () => {
    // Arrange
    req.body = { reservationId: '123' };
    ...
    expect(mockPrisma.contrat.create).not.toHaveBeenCalled();
